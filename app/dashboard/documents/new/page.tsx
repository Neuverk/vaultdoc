...
async function doDownloadWord(content: string, meta: DocMeta) {
  const lines = content.split('\n')
...
function doDownloadPDF(content: string, meta: DocMeta) {
...
export default function NewDocumentPage() {
  ...
  const dismissAiDisclosure = () => {
    try { localStorage.setItem(AI_DISCLOSURE_KEY, 'true') } catch { /* ignore */ }
...
${form.tools ? `Tools/Systems: ${form.tools}` : ''}`,
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.ready) {
        setMessages((prev) => [...prev, { role: 'assistant', content: '✅ I have everything I need. Generating your document now...' }])
        setIsThinking(false)
  ...
  const generateDocument = async (meta: Record<string, unknown>, conversationMessages: Message[]) => {
    setPhase('generating')
    try {
      ...
      setPhase('chat')
      setMessages((prev) => [...prev, { role: 'assistant', content: 'Document generation failed. Please try again.' }])
    }
  }
  ...
  const downloadWord = () => doDownloadWord(content, form)
  const downloadPDF = () => doDownloadPDF(content, form)
  ...
  const uploadFile = async (file: File) => {
    ...
    fd.append('file', file)
    try {
      const res = await fetch('/api/documents/reference-upload', { method: 'POST', body: fd })
      const data = await res.json()
      if (!res.ok) {
  ...
  const showTabBar = (tab === 'scratch' && phase === 'form') || (tab === 'reference' && refPhase === 'input')
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="mx-auto flex max-w-7xl items-center gap-3 px-6 py-4">
          ...
          {(scratchDone || refDone) && (
            <div className="ml-auto flex flex-wrap items-center gap-2">
              ...
              <button
                onClick={() => scratchDone ? downloadWord() : doDownloadWord(refContent, refSaveForm)}
              ...
              <button
                onClick={() => scratchDone ? downloadPDF() : doDownloadPDF(refContent, refSaveForm)}
                className="rounded-xl border border-gray-300 bg-white px-4 py-2 text-xs font-semibold text-gray-700 transition hover:bg-gray-50"
              >
                PDF
              </button>
              <button
                onClick={() => navigator.clipboard.writeText(scratchDone ? content : refContent)}
                className="rounded-xl border border-gray-300 bg-white px-4 py-2 text-xs font-semibold text-gray-700 transition hover:bg-gray-50"
              >
                Copy
              </button>
        ...
        <div className="mx-auto max-w-5xl px-6 py-10 lg:py-12">
          <div className="mb-8 rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
            <div className="max-w-2xl">
              ...
              <h1 className="text-2xl font-semibold tracking-tight text-gray-900">Create new document</h1>
            ...
            <div className="mb-8 rounded-3xl border border-amber-200 bg-amber-50 p-6 shadow-sm">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="max-w-3xl">
                  <p className="text-sm font-semibold text-amber-950">AI processing notice</p>
                  <p className="mt-2 text-sm leading-6 text-amber-900">
                    Your inputs are processed by Anthropic&apos;s AI to generate documents. Avoid including passwords, personal data, or classified information.
                  </p>
                </div>
                <button
                  type="button"
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
            <div className="grid gap-8">
              ...
              <div>
                <label className="mb-3 block text-sm font-medium text-gray-700">Frameworks</label>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div>
                  <label className="mb-2 block text-sm font-medium text-gray-700">Scope (optional)</label>
                  <textarea
                    value={form.scope}
                ...
                <div>
                  <label className="mb-2 block text-sm font-medium text-gray-700">Tools / systems</label>
                  <textarea
                    value={form.tools}
              ...
              <div className="flex flex-col gap-3 border-t border-gray-100 pt-6 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm text-gray-500">VaultDoc will guide the next step through a short AI follow-up flow.</p>
                <button
                  onClick={startChat}
      ...
      {tab === 'scratch' && (phase === 'chat' || phase === 'generating') && (
        <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col px-6 py-8">
          <div className="mb-6 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
            <h1 className="text-xl font-semibold tracking-tight text-gray-900">AI document follow-up</h1>
            <p className="mt-2 text-sm leading-6 text-gray-600">
              Answer the follow-up questions below. The more precise your answers, the better the final document.
            </p>
          </div>
          ...
          <div className="flex-1 space-y-5 overflow-y-auto rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-2xl rounded-2xl px-5 py-4 text-sm leading-7 whitespace-pre-wrap ${
                  msg.role === 'user'
          ...
          <p className="mt-3 text-center text-xs text-gray-400">Press Enter to send · Shift+Enter for new line</p>
        </div>
      )}
        ...
        <div className={`mx-auto w-full px-6 py-8 ${fullscreen ? 'max-w-full' : 'max-w-7xl'}`}>
          <div className="mb-5 flex flex-col gap-4 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600">Review, edit, save, and export your generated document.</p>
            </div>
            <button
        ...
        <div className="mx-auto max-w-5xl px-6 py-10 lg:py-12">
          <div className="mb-8 rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
            <div className="max-w-2xl">
              ...
              <h1 className="text-2xl font-semibold tracking-tight text-gray-900">Create from reference</h1>
              <p className="mt-2 text-sm leading-6 text-gray-600">
                Paste an existing document, draft, or raw notes. VaultDoc will analyse it first, then let you choose what to do with it.
              </p>
            </div>
          </div>
            ...
            <div className="grid gap-6">
              <div>
                ...
                <div className="mt-1.5 flex items-center justify-between">
                  <p className="text-xs text-gray-400">{refText.length.toLocaleString()} characters</p>
                  {refUploadError && (
              ...
              <div className="flex justify-end border-t border-gray-100 pt-4">
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-xl bg-gray-900 px-6 py-3 text-sm font-semibold text-white transition hover:bg-gray-800 disabled:bg-gray-300 disabled:text-gray-500"
                >
                  Analyze reference
                </button>
              </div>
      ...
      {tab === 'reference' && refPhase === 'analyzing' && (
        <div className="mx-auto max-w-5xl px-6 py-20">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm font-medium text-gray-700">Analysing your reference document…</p>
      ...
      {tab === 'reference' && refPhase === 'analyzed' && refAnalysis && (
        <div className="mx-auto max-w-5xl px-6 py-10 lg:py-12">
          <div className="mb-6 rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
            ...
            <h2 className="text-xl font-semibold tracking-tight text-gray-900">Reference analysis</h2>
          ...
          <div className="mb-6 grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-2 text-sm leading-6 text-gray-700">{refAnalysis.summary}</p>
            </div>

            <div className="space-y-3">
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Detected type</p>
                <p className="mt-1 text-sm font-semibold text-gray-900">{refAnalysis.detectedType}</p>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Likely department</p>
                <p className="mt-1 text-sm font-semibold text-gray-900">{refAnalysis.likelyDepartment}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6 grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <p className="mb-3 text-xs font-medium uppercase tracking-wider text-gray-400">Suggested frameworks</p>
              <div className="flex flex-wrap gap-2">
                {refAnalysis.suggestedFrameworks.length > 0
                  ? refAnalysis.suggestedFrameworks.map((fw) => (
                      <span key={fw} className="inline-flex rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700">
                        {fw}
                      </span>
                    ))
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <p className="mb-3 text-xs font-medium uppercase tracking-wider text-gray-400">Identified gaps</p>
          ...
          {refAnalysis.recommendedNextAction && (
            <div className="mb-6 rounded-2xl border border-gray-200 bg-gray-50 px-5 py-4">
              <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Recommended next action</p>
              <p className="mt-1 text-sm text-gray-700">{refAnalysis.recommendedNextAction}</p>
            </div>
          )}
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {([
                { action: 'create-draft' as RefAction, label: 'Create draft', desc: 'Transform reference into a structured compliance document' },
                { action: 'find-gaps' as RefAction, label: 'Find gaps', desc: 'Generate a gap analysis report against compliance standards' },
                { action: 'improve-structure' as RefAction, label: 'Improve structure', desc: 'Reformat into a professional compliance document structure' },
                ...
                >
                  ...
                  <span className="mt-1 text-xs leading-5 text-gray-500">{desc}</span>
                </button>
              ))}
            </div>
            ...
            <div className="mt-4 flex justify-end border-t border-gray-100 pt-4">
              <button
                onClick={() => { setRefPhase('input'); setRefAnalysis(null) }}
      ...
      {tab === 'reference' && refPhase === 'generating' && (
        <div className="mx-auto max-w-5xl px-6 py-20">
          <div className="flex flex-col items-center gap-4">
            <div className="h-8 w-8 animate-spin rounded-full border-2 border-gray-900 border-t-transparent" />
            <p className="text-sm font-medium text-gray-700">Generating your document…</p>
        ...
        <div className="mx-auto w-full max-w-7xl px-6 py-8">
          <div className="mb-5 flex flex-col gap-4 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h2 className="text-lg font-semibold text-gray-900">{refSaveForm.title}</h2>
